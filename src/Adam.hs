...
type Grad p s a b = GradCon p s b => (p s -> a s -> b s) -> (a s :* b s) -> p s -> R


adamStep :: forall s p a b.
     (GradCon p s b)
  => s -> s -> s -> Grad p s a b -> (p s -> a s -> b s) -> s -> a s :* b s -> Unop (p s, p s, p s)
adamStep b1 b2 eta loss m gamma sample (p, mp, vp) =
  (p ^-^ (zipWith (*) m'' (fmap (\x -> gamma / sqrt x + eta) v'')), m', v')
  where
    ((m', v'), (m'', v'')) = adamAveraging b1 b2 mp vp g 
    g = gradR (loss m sample) p
{-# INLINE adamStep #-}
...
adamSteps :: forall s p a b f.
  ...
  => Grad p s a b -> (p s -> a s -> b s) -> s -> f (a s :* b s) -> Unop (p s)
